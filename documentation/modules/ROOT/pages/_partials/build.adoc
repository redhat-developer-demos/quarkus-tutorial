...
[source,bash,subs="+macros,+attributes"]
----
podman build -f src/main/docker/Dockerfile.jvm -t example/{quarkus-project-name}:1.0-SNAPSHOT .
----
...
[source,bash,subs="+macros,+attributes"]
----
podman build -f src/main/docker/Dockerfile.native -t example/{quarkus-project-name}:1.0-SNAPSHOT .
----
...
[source,bash,subs="+macros,+attributes"]
----
podman run -it --rm -p 8080:8080 example/{quarkus-project-name}:1.0-SNAPSHOT
----
...
[source,bash,subs="+macros,+attributes"]
----
podman push example/{quarkus-project-name}:1.0-SNAPSHOT
----
