...
public class BeerService {
    ...
    public Flowable<Beer> beers() {
         return Flowable.<Beer>create(emitter -> {
            try { 
            int page = 1;
             List<Beer> beers = beerGateway.getBeers(page); // <1>
             for (Beer beer : beers) {
                 emitter.onNext(beer); // <2>
             }

             page++;
             while(!((beers = beerGateway.getBeers(page)).isEmpty()) && !emitter.isCancelled()) { // <3>
                for (Beer beer : beers) {
                    emitter.onNext(beer);
                }
                page++;
...
<2> Emits each beer
<3> Gets beers from all pages
<4> Sends an event that the process is completed
