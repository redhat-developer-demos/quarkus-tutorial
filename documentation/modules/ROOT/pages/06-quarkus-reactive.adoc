...
* [x] How to use Reactive programming for accessing Rest API services with pagination.
...
public interface BeerGateway {
    ...
    CompletionStage<List<Beer>> getBeers(@QueryParam("page") int page);
...
<1> Creates a single thread executor for getting beers by page
<2> Gets first batch of beers
<3> Sends an event that the process is completed
<4> Emits each beer
<5> Gets beers from next page
<6> Transforms the list of beers event into event of beers
