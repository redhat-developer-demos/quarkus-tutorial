...
A successful mariadb deployment will have the following pods and services int he `{tutorial-namespace}`
...
[#quarkusp-access-db]
===== Accessing Database 
To be able to access mariadb database locally, you need to `port-forward` the remote port to local port.
...
[source,bash,subs="+macros,+attributes"]
----
# e.g. kubectl port-forward -n {tutorial-namespace} svc/mariadb --address 192.168.64.1 3306:3306
...
[source,bash,subs="+macros,+attributes"]
----
# e.g. oc port-forward -n {tutorial-namespace} svc/mariadb --address 192.168.64.1 3306:3306
...
[NOTE]
====
The server address in the adminer can be either `mariadb` the kubernetes service name of the mariadb database or 
the `<your ip address>:3306`
====

==== Build Fruit Resource

The skeleton project to get started is available in pass:[$TUTORIAL_HOME]/apps/fruits-app and it is the same you created at <<Basics and Fundamentals>>.
