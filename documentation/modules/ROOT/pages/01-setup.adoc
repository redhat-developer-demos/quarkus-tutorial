...
[WARNING]
====
If you're running locally, your only requirements are Java 8+, Maven 3.6.0+, and MariaDB if you want to try Hibernate with Panache.

GraalVM is only required if you intend to build a native image.

Docker is only required if you want to build a container or run MariaDB as a container.
====
...
| https://downloads.mariadb.org/mariadb/10.4.11/[MariaDB Windows version]
|===
--
Container Image::
+
